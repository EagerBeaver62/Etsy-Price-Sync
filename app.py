...
def get_gspread_client():
    ...
    try:
        # Secrets içindeki gsheets bilgilerini kullanarak bağlan
        creds = Credentials.from_service_account_info(st.secrets["gsheets"], scopes=scope)
        return gspread.authorize(creds)
    except Exception as e:
        st.error("Google Sheets bağlantı hatası! Secrets kısmını kontrol edin.")
        st.stop()
...
def safe_float(value, default=0.0):
    """Hem virgülü hem noktayı doğru algılayan fonksiyon"""
    try:
        if value is None or value == "": return default
        # Virgülleri noktaya çevirerek float'a dönüştür
...
def edit_product_modal(row_data, row_idx):
    with st.form(f"modal_form_{row_idx}"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            # HATAYI ÇÖZEN KISIM: text_input kullanarak virgül/nokta karmaşasını engelliyoruz
            new_gr_text = st.text_input("Gram (Örn: 5,7 veya 5.7)", value=str(row_data.get('Gr')).replace(".", ","))
        ...
        if st.form_submit_button("✅ Değişiklikleri Kaydet"):
            # Metin olarak gelen gramajı burada sayıya çeviriyoruz
            new_gr = safe_float(new_gr_text)
            updated_row = [
...
with t1:
    ...
    if not df.empty:
        ...
        for idx, row in f_df.reset_index().iterrows():
            ...
            with cols[idx % 4]:
                st.markdown(f"""
                <div style="border:1px solid #eee; border-radius:15px; padding:15px; background:white; text-align:center;">
                    <img src="data:image/jpeg;base64,{img_data}" style="width:100%; height:180px; object-fit:contain; border-radius:10px;">
                    <h4 style="font-size:14px; margin-top:10px;">{row['Ürün']}</h4>
                    <div style="background:#f1f8f6; padding:5px; border-radius:8px; margin-bottom:5px;">
                        <span style="color:#2ecc71; font-weight:bold; font-size:18px;">{fiyat_gumus:,.0f} ₺</span><br><small>Gümüş</small>
                    </div>
                    <div style="background:#fffcf0; padding:5px; border-radius:8px;">
                        <span style="color:#f39c12; font-weight:bold; font-size:18px;">{fiyat_altin:,.0f} ₺</span><br><small>14K Altın</small>
                    </div>
                    <div style="font-size:11px; color:gray; margin-top:5px;">⚖️ {gr} gr | 🎯 {kar}₺ Kar</div>
                </div>
                """, unsafe_allow_html=True)
                
                c1, c2 = st.columns(2)
                if c1.button("✏️ Düzenle", key=f"e_{idx}", use_container_width=True):
                    edit_product_modal(row, row_idx)
                if c2.button("🗑️ Sil", key=f"d_{idx}", use_container_width=True):
...
with t2:
    ...
    with st.form("yeni_urun", clear_on_submit=True):
        ...
        with c1:
            ...
            # YENİ ÜRÜNDE DE METİN GİRİŞİ KULLANDIK
            u_gr_text = st.text_input("Gram (Örn: 5,7)")
        ...
        if st.form_submit_button("Sisteme Kaydet"):
            if u_ad and u_img:
                u_gr = safe_float(u_gr_text)
                img_b64 = image_to_base64(u_img)
                yeni_row = [u_ad, "Gümüş", u_gr, u_kar, img_b64, "Yüzük", u_kap, u_lazer, u_mine]
                sheet.append_row(yeni_row)
