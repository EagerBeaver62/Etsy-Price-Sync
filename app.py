...
# --- DÜZENLEME MODALI (Gramaj Sorunu Burada Çözüldü) ---
...
def edit_product_modal(row_data, row_idx):
    with st.form(f"modal_form_{row_idx}"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            # Burayı metin kutusu yaptık ki 5,7 yazdığında 57'ye yuvarlamasın
            new_gr_str = st.text_input("Gram (Örn: 5,7)", value=str(row_data.get('Gr')).replace(".", ","))
        ...
        if st.form_submit_button("✅ Değişiklikleri Kaydet"):
            ...
            updated_row = [new_ad, row_data['Maden'], updated_gr, new_kar, row_data['GörselData'], new_kat, new_kaplama, new_lazer, new_mine]
...
# --- ANA EKRAN (Orijinal Kart Tasarımı) ---
        ...
        for idx, row in f_df.reset_index().iterrows():
            ...
            with cols[idx % 4]:
                ...
                <div style="border:1px solid #eee; border-radius:15px; padding:15px; background:white; text-align:center; margin-bottom:10px;">
                    ...
                    <h4 style="font-size:14px; margin-top:10px; height:40px;">{row['Ürün']}</h4>
                ...
                if c2.button("🗑️ Sil", key=f"s_{idx}", use_container_width=True):
...
with t2:
    with st.form("yeni_urun"):
        ...
        u_gr_str = st.text_input("Gram (Örn: 5,7)")
        ...
        if st.form_submit_button("Sisteme Kaydet"):
            if u_ad and u_img:
                ...
                sheet.append_row([u_ad, "Gümüş", safe_float(u_gr_str), u_kar, img_b64, "Yüzük", 0, 0, 0])
